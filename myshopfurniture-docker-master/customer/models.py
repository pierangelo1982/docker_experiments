...
class AddFormFatturazione(ModelForm):
    class Meta:
        ...
        fields = ['user', 'denominazione', 'piva', 'codfisc', 'indirizzo', 'cap', 'citta', 'nazione',
                'telefono', 'fax', 'e_mail', 'indirizzo_spedizione']

        

class IndirizzoSpedizione(models.Model):
    user = models.ForeignKey(User, null=True, blank=True)
    spedizione_denominazione = models.CharField('denominazione', max_length=250, null=True, blank=True)
    spedizione_indirizzo = models.CharField('indirizzo', max_length=250, null=True, blank=True)
    spedizione_cap = models.CharField('Cap', max_length=250, null=True, blank=True)
    spedizione_citta = models.CharField('Citta', max_length=250, null=True, blank=True)
    spedizione_nazione = models.CharField('nazione', max_length=250, null=True, blank=True)
    spedizione_telefono = models.CharField('Telefono', max_length=250, null=True, blank=True)
    spedizione_fax = models.CharField('Fax', max_length=250, null=True, blank=True)
    spedizione_e_mail = models.CharField('email', max_length=250, null=True, blank=True)
    spedizione_pub_date = models.DateTimeField('date published', null=True, blank=True, editable=False)

    def save(self, *args, **kwargs):
        self.spedizione_pub_date = datetime.now()
...
class AddFormIndirizzoSpredizione(ModelForm):
    class Meta:
        ...
        fields = ['user', 'spedizione_denominazione', 'spedizione_indirizzo', 'spedizione_cap', 'spedizione_citta', 'spedizione_nazione',
                'spedizione_telefono', 'spedizione_fax', 'spedizione_e_mail']
